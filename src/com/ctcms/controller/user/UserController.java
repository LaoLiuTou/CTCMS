package com.ctcms.controller.user;
...
import com.ctcms.model.user.User;
import com.ctcms.service.logs.ILogsService;
import com.ctcms.service.user.IUserService;
import com.ctcms.utils.MD5Encryption;
import com.ctcms.utils.TokenUtils;
	...
	public Map list(HttpServletRequest request, HttpServletResponse response,User user)
		...
		try {
			...
			if(page!=null&&size!=null){
				...
				paramMap.put("new_flag",user.getNew_flag());
				paramMap.put("update_flag",user.getUpdate_flag());
				paramMap.put("frozen_flag",user.getFrozen_flag());
				...
				String norole=request.getParameter("norole");
				if(norole!=null&&!norole.equals(""))
					paramMap.put("norole", norole);
	...
	public Map login(HttpServletRequest request,User user){
		...
		try {
			if(user.getUsername()!=null&&user.getPassword()!=null){
				...
				List<User> list=iUserService.selectUserByParam(paramMap);
				if(list.size()>0){
					if(list.get(0).getFrozen_flag()!=null&&list.get(0).getFrozen_flag()==1){
						resultMap.put("status", "-1");
						resultMap.put("msg", "该用户已被冻结！");
					}
					else{
						if(list.get(0).getPassword().equals(MD5Encryption.getEncryption(user.getPassword()).toLowerCase())){
							...
							tokenBean.setUserid(list.get(0).getId()+"");
		  					...
		  					//登录日志
		  					Logs logs = new Logs();
		  					logs.setUser_id(list.get(0).getId());
